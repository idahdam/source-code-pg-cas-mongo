...
# Configure logging
logging.basicConfig(filename='app_join_pg.log', filemode='w', format='%(asctime)s - %(message)s', level=logging.INFO)
...
for i in range(loop_count):
    ...
    cur.execute("SELECT td.country, c.country,  td.hs9 FROM trading_data td INNER JOIN country c ON td.country = c.code WHERE td.ym = '198801' AND td.country > 103 ORDER BY td.country DESC LIMIT 1000;")
    ...
    with open(f"pg_join/output_{i}.txt", "w") as f:
        for row in rows:
            f.write(str(row) + "\n")
